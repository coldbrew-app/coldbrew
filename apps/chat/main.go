...
func run() error {
	...
	handler := chat.NewHTTPHandler(application, natsConnection.Activity, oauth, store, config.serviceSecret, config.webURL, kickWebhook, chat.NewBoostyConnector(boosty, store), natsConnection.DeadLetters)
