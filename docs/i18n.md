...
Each semantic key contains all of its localized values. `createI18n` uses the
...
accessible names, titles, and document titles, to a `createI18n` catalog next
...
2. Every localized message in the local catalogs created with `createI18n`.
