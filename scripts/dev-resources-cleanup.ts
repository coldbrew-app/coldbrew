...
const databases =
  await $`docker compose -f compose.dev.yaml exec -T postgres psql --username=${PGUSER} --dbname=postgres --tuples-only --no-align --command="SELECT datname FROM pg_database WHERE datname ~ '^streambrew_[a-z0-9_]+$' ORDER BY datname"`.text();
